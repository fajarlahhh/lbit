...
# luckybit_hash
RUN git clone https://github.com/luckybitpay/luckybit_hash
RUN cd luckybit_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} luckybit
RUN useradd -u ${USER_ID} -g luckybit -s /bin/bash -m -d /luckybit luckybit
...
RUN mkdir /luckybit-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /luckybit-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /luckybit-src
