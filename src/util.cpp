...
const char * const BITCOIN_CONF_FILENAME = "luckybit.conf";
const char * const BITCOIN_PID_FILENAME = "luckybitd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "luckybit" is a composite category enabling all Luckybit-related debug output
                if(ptrCategory->count(std::string("luckybit"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Luckybit
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Luckybit
    // Mac: ~/Library/Application Support/Luckybit
    // Unix: ~/.luckybit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Luckybit";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Luckybit";
#else
    // Unix
    return pathRet / ".luckybit";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty luckybit.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override luckybit.conf
