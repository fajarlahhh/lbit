...
{
    return QCoreApplication::translate("luckybit-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in luckybit.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in luckybit.qrc)
...
/** Class encapsulating Luckybit Core startup and shutdown.
...
/** Main Luckybit application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // luckybit: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Luckybit Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(luckybit);
    Q_INIT_RESOURCE(luckybit_locale);
    ...
    /// 6. Determine availability of data directory and parse luckybit.conf
    ...
    // luckybit: links repeatedly have their payment requests routed to this process:
