...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Luckybit related functionality
...
/*
    ...
    allow endless transaction that would bloat Luckybit and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Luckybit specific functionality
