...
// which are part of the Luckybit repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="luckybit-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /luckybit-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "luckybitpay-luckybit/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from luckybitpay-luckybit/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'luckybitpay-luckybit/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from luckybitpay-luckybit/develop"
            }
          }

          if (hasCache) {
            sh "cd /luckybit-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /luckybit-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /luckybit-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /luckybit-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /luckybit-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /luckybit-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /luckybit-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
